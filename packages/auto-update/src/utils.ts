...
const CATEGORY_RULES: Array<{ category: string; pattern: RegExp }> = [
  { category: "Writing", pattern: /\b(write|writer|copy|content|grammar|translation|blog)\b/i },
  { category: "Image", pattern: /\b(image|photo|art|avatar|diffusion|illustration)\b/i },
  ...
  { category: "Productivity", pattern: /\b(productivity|notes|meeting|workspace|assistant)\b/i },
  { category: "Marketing", pattern: /\b(marketing|social|campaign|brand|copywriting)\b/i },
  { category: "SEO", pattern: /\b(seo|search ranking|keyword|backlink)\b/i },
  { category: "Business", pattern: /\b(crm|sales|finance|customer|operations)\b/i },
  { category: "Research", pattern: /\b(research|analysis|citation|paper|search)\b/i },
  { category: "Chatbot", pattern: /\b(chat|chatbot|assistant|agent)\b/i },
