    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      <article className="mx-auto max-w-3xl px-6 py-12 lg:px-8">
        ...
        {post.coverImageUrl ? (
          <img
            ...
            alt=""
            className="mt-8 aspect-video w-full rounded-3xl object-cover shadow-sm"
          />
        ) : null}
        ...
        <p className="mt-4 text-slate-600">
          {post.publishedAt ? new Date(post.publishedAt).toLocaleDateString(locale) : "Published"}
          {post.author?.displayName ? ` · ${post.author.displayName}` : ""}
        </p>
        {post.excerpt ? (
          <p className="mt-6 text-xl leading-8 text-slate-600">{post.excerpt}</p>
        ) : null}
        <div className="prose prose-slate mt-10 max-w-none rounded-3xl border bg-white p-8 shadow-sm">
      ...
      <section className="mx-auto max-w-5xl px-6 pb-12 lg:px-8">
        ...
        {related.length ? (
          ...
          <p className="mt-3 text-sm text-slate-600">No related posts yet.</p>
        )}
      </section>
    </main>
  );
